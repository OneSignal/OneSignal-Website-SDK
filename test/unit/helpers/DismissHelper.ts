...
test.beforeEach(() => {
  ...
  mockGetIcon();
  sinonSandbox
    .stub(OneSignal, 'initializeCoreModuleAndUserNamespace')
    .resolves();
});
...
test('push slidedown shown, on dismiss, mark push prompt as dismissed', async (t) => {
  ...
  const dismissSpy = sinonSandbox.spy(
    DismissHelper,
  ...
  await closedPromise;
  const pushDismissed = await TimedLocalStorage.getItem(
    DismissTimeKey.OneSignalNotificationPrompt,
  );
  const nonPushDismissed = await TimedLocalStorage.getItem(
    DismissTimeKey.OneSignalNonPushPrompt,
  );
...
test("if subscribed w/ expired dismiss value, category slidedown doesn't prompt", async (t) => {
  ...
  const options: AutoPromptOptions = {
    slidedownPromptOptions: minimalCategorySlidedownOptions,
  };
