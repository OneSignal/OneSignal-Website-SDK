...
test('supports version 0 of config schema (converts to version 2)', async (t) => {
  ...
  const promptOptions =
    appConfig.userConfig.promptOptions?.slidedown?.prompts[0];
...
test(
  `supports version 0 of config schema (converts to version 2) - using alternative text keys ` +
    `without "Text" postfix - slidedown type is 'push'`,
  async (t) => {
    const fakeUserConfig: AppUserConfig = {
      appId: Random.getRandomUuid(),
      autoRegister: false,
...
test("supports version 1 of config schema (converts to version 2) - slidedown type is 'push'", async (t) => {
  ...
  t.is(
    promptOptions?.text.actionMessage,
...
test('converter handles case where some settings are set in v1 format and others in v2', async (t) => {
  ...
  const appConfig = await getFinalAppConfig(fakeUserConfig);
  const promptOptionsArray =
    appConfig.userConfig.promptOptions?.slidedown?.prompts;
  const pushPrompt = PromptsHelper.getFirstSlidedownPromptOptionsWithType(
    promptOptionsArray,
    DelayedPromptType.Push,
  );
  const categoryPrompt = PromptsHelper.getFirstSlidedownPromptOptionsWithType(
    promptOptionsArray,
