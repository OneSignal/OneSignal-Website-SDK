...
} from '../../../src/managers/SubscriptionManager';
import {
  base64ToUint8Array,
  arrayBufferToBase64,
...
async function testCase(
  ...
  if (onBeforeSubscriptionManagerSubscribe) {
    await onBeforeSubscriptionManagerSubscribe(
  ...
  // Subscribe for push
  await manager.subscribeWithVapidKey(
    registration.pushManager,
...
test('uses per-app VAPID public key for Chrome', async (t) => {
  ...
  await testCase(
    ...
    async (pushManager, pushManagerSubscribeSpy) => {
      ...
      t.true(
        pushManagerSubscribeSpy
          .getCall(0)
          .calledWithExactly(expectedSubscriptionOptions),
      );
    },
  );
});
...
test('resubscribe-existing strategy uses new subscription applicationServerKey', async (t) => {
  ...
  const initialSubscriptionOptions: PushSubscriptionOptions = {
    userVisibleOnly: true,
    applicationServerKey: base64ToUint8Array(initialVapidKeys.uniquePublic)
      .buffer,
  };
  ...
  await testCase(
    ...
    async (_pushManager, pushManagerSubscribeSpy) => {
      ...
      const calledSubscriptionOptions =
        pushManagerSubscribeSpy.getCall(0).args[0];
...
test('resubscribe existing strategy unsubscribes and creates new subscription if existing subscription options are null', async (t) => {
  const initialVapidKeys = generateVapidKeys();
  const subsequentVapidKeys = generateVapidKeys();
  ...
  const subsequentSubscriptionOptions: PushSubscriptionOptions = {
    userVisibleOnly: true,
    applicationServerKey: base64ToUint8Array(subsequentVapidKeys.uniquePublic)
      .buffer,
  };
...
test('resubscribe existing strategy does not unsubscribes if options are not null', async (t) => {
  const subsequentVapidKeys = generateVapidKeys();
  let unsubscribeSpy: sinon.SinonSpy;
...
test('null applicationServerKey throws when subscribing', async (t) => {
  const manager = new SubscriptionManager(OneSignal.context, {
    safariWebId: undefined,
    appId: Random.getRandomUuid(),
  ...
  await t.throwsAsync(
    manager.subscribe.bind(null, SubscriptionStrategyKind.SubscribeNew),
    { instanceOf: Error },
  );
});
...
test('registerSubscription with an existing subsription sends player update', async (t) => {
  ...
  const deviceRecord: PushDeviceRecord =
    PushDeviceRecord.createFromPushSubscription(
  ...
  sandbox
    ...
    .resolves({ deviceId } as Subscription);
  sandbox
    .stub(subscriptionManager, 'associateSubscriptionWithEmail')
    .resolves();
  sandbox.stub(SubscriptionManager, 'isSafari').returns(false);
  sandbox.stub(Database, 'setSubscription').resolves();

  const playerUpdateSpy = sandbox.stub(
    OneSignal.context.updateManager,
    'sendPushDeviceRecordUpdate',
  );
  const playerCreateSpy = sandbox.stub(
    OneSignal.context.updateManager,
    'sendPlayerCreate',
  );
...
test('registerSubscription without an existing subsription sends player create', async (t) => {
  ...
  sandbox
    ...
    .resolves();
  sandbox
    .stub(PushDeviceRecord, 'createFromPushSubscription')
...
test('device ID is available after register event', async (t) => {
  ...
  const rawPushSubscription =
    RawPushSubscription.setFromW3cSubscription(pushSubscription);
  ...
  const playerUpdateStub = sandbox
    .stub(OneSignal.context.updateManager, 'sendPushDeviceRecordUpdate')
    .resolves();
  const playerCreateStub = sandbox
    .stub(OneSignal.context.updateManager, 'sendPlayerCreate')
    .resolves(randomPlayerId);

  await OneSignal.context.subscriptionManager.registerSubscription(
    rawPushSubscription,
  );
...
test('safari 11.1+ with service worker but not pushManager', async (t) => {
  ...
  sandbox
    .stub(OneSignal.context.serviceWorkerManager, 'getActiveState')
    .returns(ServiceWorkerActiveState.OneSignalWorker);

  t.is(
    await OneSignal.context.subscriptionManager.isSubscriptionExpiring(),
    false,
  );
});

test('subscribe new strategy creates new subscription', async (t) => {
  const initialVapidKeys = generateVapidKeys();
  const subsequentVapidKeys = generateVapidKeys();
  ...
  await testCase(
    ...
    async (pushManager, subscriptionManager) => {
      ...
      initialSubscription = await pushManager.subscribe(
...
test('subscribe new strategy unsubscribes existing subscription to create new subscription', async (t) => {
  const subsequentVapidKeys = generateVapidKeys();
  let unsubscribeSpy: sinon.SinonSpy;
...
test('subscribing records created at date in UTC', async (t) => {
  const initialVapidKeys = generateVapidKeys();
...
async function expirationTestCase(
  ...
  await testCase(
    ...
    async (_pushManager, _subscriptionManager) => {
      ...
      sandbox
        .stub(MockPushSubscription.prototype, 'expirationTime')
...
test('a subscription expiring in 30 days is not refreshed before the midpoint', async (t) => {
  const dateString = 'February 26, 2018 10:04:24 UTC';
  ...
  const newTimeBeforeMidpoint =
    subscriptionCreationTime + THIRTY_DAYS_MS / 2 - 10;
...
test('a subscription expiring in 30 days is refreshed at the midpoint', async (t) => {
  const dateString = 'February 26, 2018 10:04:24 UTC';
  const THIRTY_DAYS_MS = 1000 * 60 * 60 * 24 * 30;
  const subscriptionCreationTime = Date.parse(dateString);
...
test('a subscription expiring in 30 days is refreshed after the midpoint', async (t) => {
  const dateString = 'February 26, 2018 10:04:24 UTC';
  const THIRTY_DAYS_MS = 1000 * 60 * 60 * 24 * 30;
  const subscriptionCreationTime = Date.parse(dateString);
  const subscriptionExpirationTime = Date.parse(dateString) + THIRTY_DAYS_MS;
  const newTimeAfterMidpoint =
    Date.parse(dateString) + THIRTY_DAYS_MS / 2 + THIRTY_DAYS_MS / 2;
...
test('a subscription without a recorded creation date is always considered expired', async (t) => {
  const dateString = 'February 26, 2018 10:04:24 UTC';
  const THIRTY_DAYS_MS = 1000 * 60 * 60 * 24 * 30;
  const subscriptionCreationTime = Date.parse(dateString);
  const subscriptionExpirationTime = Date.parse(dateString) + THIRTY_DAYS_MS;
...
test('a subscription without an expiration time is never considered expired', async (t) => {
  const dateString = 'February 26, 2018 10:04:24 UTC';
  const THIRTY_DAYS_MS = 1000 * 60 * 60 * 24 * 30;
  const subscriptionCreationTime = Date.parse(dateString);
...
test('the subscription expiration time should be recorded', async (t) => {
  const initialVapidKeys = generateVapidKeys();
...
test("Service worker failed to install due to 403. Send a notification for the first user's session.", async (t) => {
  ...
  const smSpyRegisterFailed = sandbox.spy(
    subscriptionManager,
    'registerFailedSubscription',
  );
  const smSpyRegister = sandbox.spy(
    subscriptionManager,
  ...
  await t.throwsAsync(
    async () =>
      subscriptionManager.subscribe(SubscriptionStrategyKind.SubscribeNew),
    { instanceOf: ServiceWorkerRegistrationError },
  );
  t.is(smSpyRegisterFailed.calledOnce, true);
  t.is(
    smSpyRegisterFailed.getCall(0).args[0],
    SubscriptionStateKind.ServiceWorkerStatus403,
  );
  t.is(smSpyRegister.calledOnce, true);
});

test("Service worker failed to install due to 403. Not the first user's session, do not send a notification.", async (t) => {
  const context: Context = OneSignal.context;
  const serviceWorkerManager = context.serviceWorkerManager;
  const subscriptionManager = context.subscriptionManager;
  const pageViewManager = context.pageViewManager;
...
test("Service worker failed to install due to 404. Send a notification for the first user's session.", async (t) => {
  const context: Context = OneSignal.context;
  const serviceWorkerManager = context.serviceWorkerManager;
  const subscriptionManager = context.subscriptionManager;
  const pageViewManager = context.pageViewManager;
...
test("Service worker failed to install due to 404. Not the first user's session, do not send a notification.", async (t) => {
  const context: Context = OneSignal.context;
  const serviceWorkerManager = context.serviceWorkerManager;
  const subscriptionManager = context.subscriptionManager;
  const pageViewManager = context.pageViewManager;
