...
testWithAltOriginContext(
  `should get correct canonical subscription URL for development environment`,
  async (t) => {
    const config = TestEnvironment.getFakeAppConfig();
    config.subdomain = 'test';
    config.httpUseOneSignalCom = true;

    const devUrlsOsTcDomain = AltOriginManager.getCanonicalSubscriptionUrls(
      config,
      EnvironmentKind.Development,
    );
    t.is(devUrlsOsTcDomain.length, 1);
    t.is(
      devUrlsOsTcDomain[0].host,
      new URL('https://test.localhost:3001').host,
    );

    config.httpUseOneSignalCom = false;

    const devUrls = AltOriginManager.getCanonicalSubscriptionUrls(
      config,
      EnvironmentKind.Development,
    );
...
testWithAltOriginContext(
  `should get correct canonical subscription URL for staging environment`,
  async (t) => {
    ...
    const stagingUrlsOsTcDomain = AltOriginManager.getCanonicalSubscriptionUrls(
      config,
      EnvironmentKind.Staging,
    );
    t.is(stagingUrlsOsTcDomain.length, 1);
    t.is(
      stagingUrlsOsTcDomain[0].host,
      new URL(`https://test.${stagingDomain}`).host,
    );

    // staging does not support an alt domain so it should be the same.
    config.httpUseOneSignalCom = false;
    const stagingUrls = AltOriginManager.getCanonicalSubscriptionUrls(
      config,
      EnvironmentKind.Staging,
    );
...
testWithAltOriginContext(
  `should get correct canonical subscription URL when api.staging.onesignal.com is used`,
  async (t) => {
    const stagingDomain = 'staging.onesignal.com';
...
testWithAltOriginContext(
  `should get correct canonical subscription URL for production environment`,
  async (t) => {
    const config = TestEnvironment.getFakeAppConfig();
    config.subdomain = 'test';
    config.httpUseOneSignalCom = true;

    const prodUrlsOsTcDomain = AltOriginManager.getCanonicalSubscriptionUrls(
      config,
    ...
    t.is(
      prodUrlsOsTcDomain[1].host,
      new URL('https://test.onesignal.com').host,
    );

    config.httpUseOneSignalCom = false;

    const prodUrls = AltOriginManager.getCanonicalSubscriptionUrls(
      config,
      EnvironmentKind.Production,
    );
...
testWithAltOriginContext(
  `should get correct canonical subscription URL for production environment with api. prefix`,
  async (t) => {
...
function setupDiscoverAltOriginTest(t: any) {
  ...
  t.context.stubIsSubscribedToBothImpl = async function () {
    if (
      this.url.host === t.context.subdomainOneSignalHost ||
...
testWithAltOriginContext(
  `should discover alt origin to be subdomain.onesignal.com if user is already subscribed there`,
  async (t) => {
    ...
    const stubLoad = sinon
      .stub(ProxyFrameHost.prototype, 'load')
      .resolves(undefined);
    const stubIsSubscribed = sinon
      .stub(ProxyFrameHost.prototype, 'isSubscribed')
      .callsFake(t.context.stubIsSubscribedToOneSignalImpl);
    const stubRemoveDuplicatedAltOriginSubscription = sinon
...
testWithAltOriginContext(
  `should discover alt origin to be subdomain.os.tc if user is not subscribed to onesignal.com but is subscribed to os.tc`,
  async (t) => {
    setupDiscoverAltOriginTest(t);
    const { appConfig } = t.context;
    if (!appConfig) {
    ...
    const stubIsSubscribed = sinon
      .stub(ProxyFrameHost.prototype, 'isSubscribed')
      .callsFake(t.context.stubIsSubscribedToOsTcImpl);
    const stubRemoveDuplicatedAltOriginSubscription = sinon
      .stub(AltOriginManager, 'removeDuplicatedAltOriginSubscription')
      .resolves(undefined);
...
testWithAltOriginContext(
  `should discover alt origin to be subdomain.os.tc if user is not subscribed to either onesignal.com or os.tc`,
  async (t) => {
    setupDiscoverAltOriginTest(t);
    const { appConfig } = t.context;
    if (!appConfig) {
    ...
    const stubIsSubscribed = sinon
      .stub(ProxyFrameHost.prototype, 'isSubscribed')
      .callsFake(t.context.stubIsSubscribedToNoneImpl);
    const stubRemoveDuplicatedAltOriginSubscription = sinon
      .stub(AltOriginManager, 'removeDuplicatedAltOriginSubscription')
      .resolves(undefined);
...
testWithAltOriginContext(
  `should discover alt origin to be subdomain.os.tc if user is subscribed to both onesignal.com and os.tc`,
  async (t) => {
    setupDiscoverAltOriginTest(t);
    const { appConfig } = t.context;
    if (!appConfig) {
    ...
    const stubIsSubscribed = sinon
      .stub(ProxyFrameHost.prototype, 'isSubscribed')
      .callsFake(t.context.stubIsSubscribedToBothImpl);
    const stubUnsubscribeFromPush = sinon
      .stub(ProxyFrameHost.prototype, 'unsubscribeFromPush')
      .resolves(undefined);
