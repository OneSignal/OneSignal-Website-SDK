...
test('sendOnSessionUpdateFromPage, makes on_session call for existing email player', async (t) => {
  ...
  const onSessionForEmail =
    NockOneSignalHelper.nockPlayerOnSession(emailPlayerId);

  const sessionManager = new SessionManager(OneSignal.context);
  await sessionManager.sendOnSessionUpdateFromPage();

  t.is(
    (await onSessionForEmail.result).request.url,
    `/api/v1/players/${emailPlayerId}/on_session`,
  );
  t.deepEqual((await onSessionForEmail.result).request.body, {
    app_id: OneSignal.config.appId,
    device_model: '',
