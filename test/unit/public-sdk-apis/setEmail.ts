...
async function setEmailTest(t: ExecutionContext, testData: SetEmailTestData) {
  ...
  t.deepEqual(
    finalEmailProfile.identifierAuthHash,
    testData.identifierAuthHash,
  );
...
test('Existing push subscription, existing identical email, with identifierAuthHash, refreshing setEmail call', async (t) => {
  const existingEmailId = Random.getRandomUuid();
  const testData = {
    existingEmailAddress: 'existing-different-email@example.com',
    newEmailAddress: 'test@example.com',
    identifierAuthHash:
      '432B5BE752724550952437FAED4C8E2798E9D0AF7AACEFE73DEA923A14B94799',
    existingEmailId: existingEmailId,
    newEmailId: existingEmailId,
    externalUserIdAuthHash: null,
  } as SetEmailTestData;
...
test("Setting email causes 'emailSubscriptionChanged' event to fire with email identifier in event callback", async (t) => {
