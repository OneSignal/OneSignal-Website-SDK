...
test('setExternalUserId before email, includes external_user_id in POST create call', async (t) => {
  ...
  NockOneSignalHelper.nockPlayerPut(
    (await emailPostNock.result).response.body.id,
  );
