...
test('check conversion from tag boolean values to number strings', (t) => {
  const converted = TagUtils.convertTagsBooleansToApi({
    tag1: false,
    tag2: true,
  });
...
test('check that getCheckedTagCategories returns correct tagCategory list', (t) => {
  ...
  const checked = TagUtils.getCheckedTagCategories(
...
test('check that limitCategoriesToMaxCount returns correct category object', (t) => {
  ...
  const limitedUnder = TagUtils.limitCategoriesToMaxCount(
    categoriesUnderMaxTags,
    maxCount,
  );
  const limitedOver = TagUtils.limitCategoriesToMaxCount(
    categoriesOverMaxTags,
    maxCount,
  );
