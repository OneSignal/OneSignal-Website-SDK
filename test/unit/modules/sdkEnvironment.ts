...
test('getIntegration should return SecureProxy in HTTPS child frame using proxy origin under top-level HTTPS frame', async (t) => {
  const browser = await TestEnvironment.stubDomEnvironment();
  browser.reconfigureWindow(window, { top: 'another frame' as any });
  browser.changeURL(window, 'https://site.com');
  const stub = mockParentFrame(true);
...
test('getIntegration should return InsecureProxy in HTTP top-level frame using proxy origin', async (t) => {
  ...
  t.is(
    await SdkEnvironment.getIntegration(true),
...
test('getIntegration should return InsecureProxy in HTTP top-level frame not using proxy origin', async (t) => {
  ...
  browser.changeURL(window, 'http://site.com');
  t.is(
    await SdkEnvironment.getIntegration(false),
    IntegrationKind.InsecureProxy,
  );
});
...
test('getIntegration should return InsecureProxy in HTTP top-level frame for not localhost and allowLocalhostAsSecureOrigin = true', async (t) => {
  const OneSignal: any = await TestEnvironment.initialize({
    httpOrHttps: HttpHttpsEnvironment.Http,
  });
  TestEnvironment.mockInternalOneSignal();
...
test('getIntegration should return InsecureProxy in HTTP top-level frame for 127.0.0.1 and allowLocalhostAsSecureOrigin not present', async (t) => {
  const OneSignal: any = await TestEnvironment.initialize({
    httpOrHttps: HttpHttpsEnvironment.Http,
  });
  TestEnvironment.mockInternalOneSignal();
...
test('getIntegration should return Secure in HTTP top-level frame for 127.0.0.1 and allowLocalhostAsSecureOrigin = true', async (t) => {
  const OneSignal: any = await TestEnvironment.initialize({
    httpOrHttps: HttpHttpsEnvironment.Http,
  });
  TestEnvironment.mockInternalOneSignal();
...
test('getIntegration should return InsecureProxy in HTTP top-level frame for localhost and allowLocalhostAsSecureOrigin not present', async (t) => {
  const OneSignal: any = await TestEnvironment.initialize({
    httpOrHttps: HttpHttpsEnvironment.Http,
  });
  TestEnvironment.mockInternalOneSignal();
...
test('getIntegration should return Secure in HTTP top-level frame for localhost and allowLocalhostAsSecureOrigin = true', async (t) => {
  const OneSignal: any = await TestEnvironment.initialize({
    httpOrHttps: HttpHttpsEnvironment.Http,
  });
  TestEnvironment.mockInternalOneSignal();
