...
class Defaults {
  public static delayedFunctionCall = {
    functionName: '',
    args: [],
...
class OneSignalStubES5Test extends OneSignalStubES5 {
  public lastDirectFunctionCall: DelayedFunctionCall<any> =
    Defaults.delayedFunctionCall;
  public lastDirectPromiseFunctionCall: DelayedFunctionCall<any> =
    Defaults.delayedFunctionCall;

  protected stubFunction(
    thisObj: OneSignalStubES5Test,
    functionName: string,
    args: any[],
  ): any {
    thisObj.lastDirectFunctionCall = {
  ...
  protected stubPromiseFunction(
    ...
    thisObj.lastDirectPromiseFunctionCall = {
      functionName: functionName,
      args: args,
      delayedPromise: undefined,
    };
...
function assertES5MethodIsCalled(
  t: ExecutionContext,
  oneSignalStub: OneSignalStubES5Test,
  functionName: string,
) {
  const retValue = (oneSignalStub as any)[functionName].call(
    null,
    `${functionName}:arg1`,
  );
...
function assertES5PromiseMethodIsCalled(
  t: ExecutionContext,
  oneSignalStub: OneSignalStubES5Test,
  functionName: string,
) {
  ...
  t.is(
    oneSignalStub.lastDirectPromiseFunctionCall.args[0],
    `${functionName}:arg1`,
  );
...
class OneSignalStubES6Test extends OneSignalStubES6 {
  public getLastStubCall(): DelayedFunctionCall<any> {
    return this.directFunctionCallsArray[
      this.directFunctionCallsArray.length - 1
    ];
  }
}

function assertES6MethodIsCalled(
  t: ExecutionContext,
  oneSignalStub: OneSignalStubES6Test,
  functionName: string,
) {
  const retValue = (oneSignalStub as any)[functionName].call(
    null,
...
function assertES6PromiseMethodIsCalled(
  t: ExecutionContext,
  oneSignalStub: OneSignalStubES6Test,
  functionName: string,
) {
...
test('Test ReplayCallsOnOneSignal replays ES6 calls with expected params using push with params list', async (t) => {
  ...
  const sendTagsPromise = (oneSignalStub as any).push([
    'sendTag',
    'key1',
    'value2',
  ]);
...
test(
  `Test ReplayCallsOnOneSignal replays ES6 calls from preExistingArray with ` +
    `expected params with a function`,
  async (t) => {
...
test(
  `Test ReplayCallsOnOneSignal replays ES6 calls from preExistingArray with ` +
    `expected params using push with params list`,
  async (t) => {
    // Setup an OneSignalStubES6 instance like the OneSignalSDK.js Shim does.
...
class MockOneSignalWithPromiseControl {
  ...
  async sendTag(
    ...
    return new Promise((resolve, reject) => {
      if (this.resolveValue) resolve(this.resolveValue);
      if (this.rejectValue) {
...
test('Expect Promise to never resolve for ES5 stubs', async (t) => {
  ...
  const sendTagsPromise = (oneSignalStub as any).sendTag('key', 'value');
  sendTagsPromise.then(() => t.fail()).catch(() => t.fail());
  t.pass();
});
...
test('OneSignalSDK.js load from service worker context that supports push', async (t) => {
  ...
  t.true(
    importScriptsSpy
      .getCall(0)
      .calledWithExactly(
        'https://cdn.onesignal.com/sdks/OneSignalSDKWorker.js?v=1',
      ),
  );
});
...
test('Existing OneSignal array before OneSignalSDK.js loaded ES6', async (t) => {
  ...
  t.deepEqual(
    <object[]>(<OneSignalStubES6>(<any>window).OneSignal).preExistingArray,
    preExistingArray,
  );
});
