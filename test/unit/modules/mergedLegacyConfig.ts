...
test.beforeEach((t) => {
  t.context.overrideServerConfig = TestEnvironment.getFakeServerAppConfig(
...
test('should not overwrite a provided service worker parameters', async (t) => {
  ...
  t.deepEqual(result.userConfig.serviceWorkerParam, {
    scope: '/existing-path',
  });
  t.is(
    result.userConfig.serviceWorkerPath,
    '/existing-path/OneSignalSDKWorker.js',
  );
});
...
test('should assign downloaded safari web ID if not provided', async (t) => {
  ...
  serverConfig.config.safari_web_id =
    'web.onesignal.auto.01ea4289-b460-45e4-8d90-838752554827';
  const result = new ConfigManager().getMergedConfig({}, serverConfig);
