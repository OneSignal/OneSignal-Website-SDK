...
export default class EventsTestHelper {
  ...
  public getShownPromiseWithEventCounts(
    resolveAfter: number = 0,
  ): Promise<void> {
    return new Promise<void>((resolve) => {
      OneSignal.on(Slidedown.EVENTS.SHOWN, () => {
        this.eventCounts.shown += 1;
        if (this.eventCounts.shown >= resolveAfter) {
          resolve();
        }
      });
    });
  ...
  public getClosedPromiseWithEventCounts(
    resolveAfter: number = 0,
  ): Promise<void> {
    return new Promise<void>((resolve) => {
      OneSignal.on(Slidedown.EVENTS.CLOSED, () => {
        this.eventCounts.closed += 1;
        if (this.eventCounts.closed >= resolveAfter) {
          resolve();
        }
      });
    });
  ...
  public simulateSubscribingAfterNativeAllow() {
    OneSignal.emitter.on(OneSignal.EVENTS.PERMISSION_PROMPT_DISPLAYED, () => {
      this.sinonSandbox
        .stub(OneSignal, 'privateGetSubscription')
        .resolves(true);
      this.sinonSandbox
