...
function setBuildEnvironment() {
  ...
  console.info('Build Environment:', process.env.ENV);
}
...
async function getWebpackPlugins() {
  const plugins = [
    ...
    new MiniCssExtractPlugin({
      filename: 'OneSignalSDK.page.styles.[contenthash].css',
    }),
...
async function generateWebpackConfig() {
  return {
    ...
    entry: {
      ...
      'OneSignalSDK.sw.js': path.resolve(
...
function getStylesheetsHash() {
  ...
  if (files.length === 0) {
    throw new Error(
      `No .scss files were found in ${styleSheetsPath}, but SCSS files were expected.`,
    );
  }
