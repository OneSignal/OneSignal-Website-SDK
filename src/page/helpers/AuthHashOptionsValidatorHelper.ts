...
export class AuthHashOptionsValidatorHelper {
  ...
  static throwIfInvalidAuthHashOptions(
    options: any,
    keys: Array<string>,
  ): string | undefined {
    if (!options) {
    ...
    if (validKeys.length > 1) {
      Log.error(
        'More than one key provided, please only provide one!',
        validKeys,
      );
    ...
    AuthHashOptionsValidatorHelper.throwIfInvalidAuthHash(
      hashToCheck,
      `options.${keyName}`,
    );
  ...
  static throwIfInvalidAuthHash(
    ...
    if (
      value.length !== AuthHashOptionsValidatorHelper.VALID_AUTH_HASH_LENGTH
    ) {
      throw new InvalidArgumentError(
        fieldName,
        InvalidArgumentReason.Malformed,
