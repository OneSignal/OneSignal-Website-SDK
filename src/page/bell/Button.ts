...
export default class Button extends AnimatedElement {
  ...
  _onTap() {
    this._pulse();
    this._activate();
    this._bell.__badge._activate();
  }

  _onEndTap() {
    this._inactivate();
    this._bell.__badge._inactivate();
  }
  ...
  async _onClick() {
    ...
    try {
      ...
      if (
        this._bell.__message._shown &&
        this._bell.__message._contentType == MessageType._Message
      ) {
      ...
      if (
        ...
        await this._bell.__launcher._activateIfInactive();
        await this._toggleDialog();
      }

      await this._bell.__message._hide();
    } finally {
  ...
  private async _toggleDialog() {
    if (this._bell.__dialog._shown) {
      // Close dialog if already open (toggle behavior)
      await this._bell.__dialog._hide();
      await this._bell.__launcher._inactivateIfWasInactive();
    } else {
