...
export default class Dialog extends AnimatedElement {
  public _bell: Bell;
  public _subscribeButtonId: string;
  public _unsubscribeButtonId: string;
  ...
  get _subscribeButton() {
    return this._element
      ? this._element.querySelector('#' + SUBSCRIBE_BUTTON_ID)
      : null;
  }

  get _unsubscribeButton() {
    return this._element
      ? this._element.querySelector('#' + UNSUBSCRIBE_BUTTON_ID)
      : null;
  }
  ...
  _updateBellLauncherDialogBody() {
    return OneSignal._context._subscriptionManager
      ...
      .then((currentSetSubscription: boolean) => {
        ...
        if (
          (this._bell._state === Bell._STATES._SUBSCRIBED &&
            currentSetSubscription === true) ||
          (this._bell._state === Bell._STATES._UNSUBSCRIBED &&
          ...
          let buttonHtml = '';
          if (this._bell._state !== Bell._STATES._SUBSCRIBED)
            buttonHtml = `<button type="button" class="action" id="${SUBSCRIBE_BUTTON_ID}">${this._bell._options.text['dialog.main.button.subscribe']}</button>`;
          else
            buttonHtml = `<button type="button" class="action" id="${UNSUBSCRIBE_BUTTON_ID}">${this._bell._options.text['dialog.main.button.unsubscribe']}</button>`;
        ...
        } else if (this._bell._state === Bell._STATES._BLOCKED) {
          let imageUrl = null;
