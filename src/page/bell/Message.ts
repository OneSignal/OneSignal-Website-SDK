...
export default class Message extends AnimatedElement {
  ...
  _enqueue(message: string) {
    ...
    return new Promise<void>((resolve) => {
      if (this._bell.__badge._shown) {
        this._bell.__badge
          ._hide()
          .then(() => this._bell.__badge._increment())
          .then(() => this._bell.__badge._show())
          .then(() => resolve());
      } else {
        this._bell.__badge._increment();
        if (this._bell._initialized)
          this._bell.__badge._show().then(() => resolve());
        else resolve();
      }
    });
  }
  ...
  _dequeue() {
    ...
    return new Promise((resolve) => {
      if (this._bell.__badge._shown) {
        this._bell.__badge
          ._hide()
          .then(() => this._bell.__badge._decrement())
          .then(() => {
            const numMessagesLeft = Number(this._bell.__badge._content) || 0;
            if (numMessagesLeft > 0) {
              return this._bell.__badge._show();
            } else {
