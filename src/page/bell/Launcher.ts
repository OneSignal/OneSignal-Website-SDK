...
export default class Launcher extends AnimatedElement {
  ...
  _inactivate() {
    return this._bell._message.hide().then(() => {
      if (this._bell._badge.content.length > 0) {
        return this._bell._badge
          .hide()
          .then(() => Promise.all([super._inactivate(), this._resize('small')]))
          .then(() => this._bell._badge.show());
      } else {
        return Promise.all([super._inactivate(), this._resize('small')]);
      }
    });
  }
