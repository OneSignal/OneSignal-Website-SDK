...
async function onSubscriptionChanged_evaluateNotifyButtonDisplayPredicate() {
  if (!OneSignal._config?.userConfig.notifyButton) return;
  ...
  if (
    ...
    if (predicateResult !== false) {
      ...
      OneSignal._notifyButton.launcher.show();
    } else {
      Log._debug(
        'Hiding notify button because display predicate returned false.',
      );
      OneSignal._notifyButton.launcher.hide();
    }
  }
}

function onSubscriptionChanged_updateCustomLink() {
  if (OneSignal._config?.userConfig.promptOptions) {
    new CustomLinkManager(
      OneSignal._config?.userConfig.promptOptions.customlink,
    ).initialize();
  }
}
...
async function onSubscriptionChanged_showWelcomeNotification(
  ...
  const welcome_notification_opts =
    OneSignal._config?.userConfig.welcomeNotification;
