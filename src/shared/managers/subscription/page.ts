...
import { OperationFailureReason } from 'src/core/types/operation';
...
async function createSubscribedUser(pushModel: SubscriptionModel): Promise<void> {
  ...
  try {
    ...
    if (e instanceof OperationFailedError && e.reason === OperationFailureReason._Suppressed) {
      return;
    }
    throw e;
  }
...
export class SubscriptionManagerPage extends SubscriptionManagerBase<ContextInterface> {
  ...
  async _updatePushSubscriptionNotificationTypes(
    ...
    if (!pushModel) {
      Log._info('No push sub to update notification_types');
      return;
    }
  ...
  async _subscribeFcmFromPage(
    ...
    */
    if (Notification.permission === 'default') {
      ...
      switch (permission) {
        case 'default':
          Log._debug('Permission dismissed, exiting subscription');
          OneSignal._sessionInitAlreadyRunning = false;
          throw new Error('Permission dismissed');
        case 'denied':
          Log._debug('Permission blocked, exiting subscription');
          OneSignal._sessionInitAlreadyRunning = false;
          throw PermissionBlockedError;
      }
    }
