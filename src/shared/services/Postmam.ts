...
/**
 * Establishes a cross-domain MessageChannel between the current browsing context (this page) and another
 *  (an iFrame, popup, or parent page).
 */
export default class Postmam {
  ...
  public messagePort: MessagePort | null;
   ...
   * @param sendToOrigin The origin that will receive the initial postMessage with the transferred message channel
   * port object.
   * @param receiveFromOrigin The origin to allow incoming messages from. If messages do not come from this origin
   * they will be discarded. Only affects the initial handshake.
   * @remarks The initiating (client) page must call this after the page has been loaded so that the other page has
   * a chance to receive the initial handshake message. The receiving (server) page must set up a message listener
   * to catch the initial handshake message.
   */
  constructor(
    ...
    if (!window || !window.postMessage) {
      throw new Error(
        `Must pass in a valid window reference supporting postMessage(): ${windowReference}`,
      );
    }
  ...
  /**
   * Opens a message event listener to listen for a Postmam handshake from another browsing context. This
   * listener is closed as soon as the connection is established.
   */
  listen() {
  ...
  onWindowMessagePostmanConnectReceived(e) {
    ...
    Log.debug(
      `(Postmam) (${env}) Window postmessage for Postman connect received:`,
      e,
    );
    // Discard messages from unexpected origins; messages come frequently from other origins
    if (!this.isSafeOrigin(e.origin)) {
  ...
  connect() {
    Log.info(
      `(Postmam) (${SdkEnvironment.getWindowEnv().toString()}) Establishing a connection to ${
        this.sendToOrigin
      }.`,
    );
  ...
  onMessageReceived(e) {
    ...
    if (!e.data) {
      Log.debug(
        `(${SdkEnvironment.getWindowEnv().toString()}) Received an empty Postmam message:`,
        e,
      );
      return;
    }
  ...
  isSafeOrigin(messageOrigin) {
    ...
    if (!OneSignal.config) {
      ...
      subdomain = OneSignal.config.subdomain as string;
    }

    const otherAllowedOrigins = this.generateSafeOrigins(
      this.receiveFromOrigin,
    );

    return (
      // messageOrigin === '' || TODO: See if messageOrigin can be blank
