...
export default class Dialog extends AnimatedElement {
  ...
  updateBellLauncherDialogBody() {
    return OneSignal.getSubscription().then(
      (currentSetSubscription: boolean) => {
        ...
        if (
          ...
          } else if (bowser.firefox) imageUrl = '/bell/firefox-unblock.jpg';
          else if (bowser.safari) imageUrl = '/bell/safari-unblock.jpg';
          else if (bowser.msedge) imageUrl = '/bell/edge-unblock.png';
