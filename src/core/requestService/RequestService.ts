...
export class RequestService {
  ...
  static identifyUser(alias: AliasPair, identity: IdentityModel): Promise<IdentityModel> {
    ...
    if (isIdentityModel(returnedIdentity)) {
   ...
   */
  static createSubscription(alias: AliasPair, subscription: SubscriptionModel): Promise<SubscriptionModel> {
    const returnedSubscription = OneSignalApiBase.post(`user/by/${alias.label}/${alias.id}/subscription`, subscription);

    if (isSubscriptionModel(returnedSubscription)) {
   ...
   * @param subscriptionId - subscription id
   */
  static deleteSubscription(subscriptionId: string): Promise<any> {
