...
export class ServiceWorker {
      ...
      switch (data.command) {
        case WorkerMessengerCommand.SessionUpsert:
          ...
          ServiceWorker.debounceRefreshSession(
            event,
            payload as UpsertSessionPayload,
          );
          break;
        case WorkerMessengerCommand.SessionDeactivate:
          ...
          ServiceWorker.debounceRefreshSession(
            event,
            payload as DeactivateSessionPayload,
          );
          break;
        default:
          return;
      ...
      ServiceWorker.parseOrFetchNotifications(event)
        .then(async (notifications: any) => {
          ...
          for (const rawNotification of notifications) {
            ...
            const notification =
              ServiceWorker.buildStructuredNotificationObject(rawNotification);
            ...
            notificationReceivedPromises.push(
              Database.put('NotificationReceived', notificationReceived),
            );
            ...
            notificationEventPromiseFns.push(
              ((notif) => {
                return ServiceWorker.displayNotification(notif)
                  .then(() => {
                    return ServiceWorker.workerMessenger
                      .broadcast(
                        WorkerMessengerCommand.NotificationDisplayed,
                        notif,
                      )
                      .catch((e) => Log.error(e));
                  })
                  .then(() =>
                    ServiceWorker.executeWebhooks(
  ...
  static async sendConfirmedDelivery(
    ...
    return await OneSignalApiBase.put(
      `notifications/${notification.id}/report_received`,
      postData,
    );
  }
  ...
  static async onNotificationClicked(event: NotificationEventInit) {
    ...
    if (event.action) notificationData.action = event.action;
    ...
    const launchUrl: string = await ServiceWorker.getNotificationUrlToOpen(
      notificationData,
    );
  ...
  static async sendConvertedAPIRequests(
    ...
    await ServiceWorker.executeWebhooks(
  ...
  static _getTitle() {
    return new Promise((resolve) => {
      Promise.all([
        ...
        Database.get('Options', 'pageTitle'),
