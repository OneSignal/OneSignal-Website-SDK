...
export default class SdkEnvironmentHelper {
  /**
   ...
   * endpoint.
   */
  public static getOneSignalApiUrl(buildEnv: BuildEnvironmentKind = SdkEnvironmentHelper.getBuildEnv()): URL {
    switch (buildEnv) {
        ...
        if (location.href.indexOf("initOneSignal") !== -1 ||
          ...
          (
            ...
            (location.hostname.indexOf('.localhost') !== -1 &&
              SdkEnvironmentHelper.getBuildEnv() === BuildEnvironmentKind.Development)
          )
        ) {
   ...
   */
  public static getBuildEnvPrefix(buildEnv: BuildEnvironmentKind = SdkEnvironmentHelper.getBuildEnv()) : string {
    switch (buildEnv) {
      case BuildEnvironmentKind.Development:
        return 'Dev-';
      case BuildEnvironmentKind.Staging:
   ...
   *
   * @param isSafari Flag indicating if it's safari specific flow.
  ...
  public static async getIntegration(usingProxyOrigin?: boolean): Promise<IntegrationKind> {
    ...
    if (isTopFrame) {
      ...
      if (isHttpsProtocol) {
        ...
        const isFrameContextInsecure = await SdkEnvironmentHelper.isFrameContextInsecure();
