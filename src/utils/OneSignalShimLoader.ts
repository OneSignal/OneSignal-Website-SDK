...
export class OneSignalShimLoader {
    ...
    switch (__BUILD_TYPE__) {
      case 'development':
        return __NO_DEV_PORT__
          ? `${protocol}://${buildOrigin}/sdks/Dev-`
          : `${protocol}://${buildOrigin}:${port}/sdks/Dev-`;
      case 'staging':
  ...
  private static addOneSignalPageES6SDKStub(): void {
    const predefinedOneSignal: PossiblePredefinedOneSignal = (<any>window)
      .OneSignal;
  ...
  public static start(): void {
    ...
    if (OneSignalShimLoader.isServiceWorkerRuntime()) {
      (<ServiceWorkerGlobalScope>(<any>self)).importScripts(
        `${OneSignalShimLoader.getPathAndPrefix()}OneSignalSDKWorker.js?v=${
          OneSignalShimLoader.VERSION
        }`,
      );
    } else if (isPushNotificationsSupported()) {
      OneSignalShimLoader.addScriptToPage(
        `${OneSignalShimLoader.getPathAndPrefix()}OneSignalPageSDKES6.js?v=${
          OneSignalShimLoader.VERSION
        }`,
      );
