...
export class ReplayCallsOnOneSignal {
  public static doReplay(
    stubOneSignal: OneSignalStubES6 | object[] | undefined | null,
  ) {
    if (!stubOneSignal) return;

    if (Array.isArray(stubOneSignal)) {
      ReplayCallsOnOneSignal.processAsArray(stubOneSignal);
    } else if (
      stubOneSignal.constructor &&
      stubOneSignal.constructor.name === 'OneSignalStubES6'
    ) {
