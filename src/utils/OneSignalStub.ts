...
export type PossiblePredefinedOneSignal =
...
export abstract class OneSignalStub<T> implements IndexableByString<any> {
  ...
  private static FUNCTION_LIST_TO_STUB = ['on', 'off', 'once', 'push'];
  ...
  protected abstract stubFunction(
    thisObj: T,
    functionName: string,
    args: any[],
  ): any;
  protected abstract stubPromiseFunction(
    thisObj: T,
    functionName: string,
    args: any[],
  ): Promise<any>;
  ...
  protected constructor(omitStubsFor: Array<string>) {
    ...
    this.setupStubFunctions(
      ...
      this.stubPromiseFunction,
      omitStubsFor,
    );
  }

  private setupStubFunctions(
    stubList: Array<string>,
    stubFunction: Function,
    omitStubsFor: Array<string>,
  ) {
    for (const functionName of stubList) {
      if (omitStubsFor.indexOf(functionName) > -1) continue;

      const functionNameWithStub = (...args: any[]): any => {
