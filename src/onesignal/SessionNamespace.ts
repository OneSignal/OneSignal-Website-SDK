...
export class SessionNamespace {
  ...
  async sendUniqueOutcome(outcomeName: string): Promise<void> {
    ...
    const newNotifsToAttributeWithOutcome =
      await outcomesHelper._getNotifsToAttributeWithUniqueOutcome(notificationIds);

    if (!outcomesHelper._shouldSendUnique(outcomeAttribution, newNotifsToAttributeWithOutcome)) {
